...
- [x] Smoke deployable: a thrown canister leaves five overlapping, enterable opaque circles that block sight without physical collision; from inside, only the union of circles containing the viewer remains unshadowed. One compact collider expands into all five in the shader, and one static graphics object replaces eighteen animated children ([smoke.go](server/internal/game/smoke.go), [smoke.ts](web/src/game/smoke.ts), `World.visionOccluded`)
...
Smoke and flashbangs did not wait for 2.6. Smoke expands its authored field
radius into five shared sight circles without pretending to be solid; entering
one changes the visible region to the union of circles containing the viewer.
...
- [x] Shared substrate for smoke and for the Mage/Gunslinger LOS matchup: smoke fields and terrain sightlines feed the same silhouette-aware targeting predicate, while manual ground placement remains exempt ([combat.md](docs/game/design/combat.md#time-to-kill))
- [x] GPU-computed client shadow wedges derived from snapshot colliders, including smoke; all shipped entities except players/projectiles remain beneath the 27% veil, which darkens covered portions of bodies and accessories, while server silhouette sampling retains a player/projectile if any part is visible and a 140 ms render-only fade softens transitions ([game-view-and-hud.md](docs/game/ui/game-view-and-hud.md#camera))

LOS is authoritative absence rather than a client mask for players and
projectiles: they are omitted only when center plus perimeter samples are all
covered, while every other shipped entity stays present beneath the veil.
Homing and chain acquisition use the same partial-silhouette rule and fall
through fully hidden targets. Destroyed or expired occluders stop blocking
immediately. The client darkens the matching pixels—including covered portions
of retained bodies and accessories—and fades actor containers without retaining
stale interpolation state.
