...
- [x] Smoke deployable: a thrown canister leaves a standing field that blocks sight without physical collision, preserves its close-contact reveal, and supplies circle geometry to the shadow shader, drawn as drifting particles ([deployable.go](server/internal/game/deployable.go), `World.visionOccluded`, [gadgets.json](data/tuning/gadgets.json))
...
Smoke and flashbangs did not wait for 2.6. Smoke now contributes its authored
field radius to the shared sightline and automatic-target checks without
pretending to be solid; its containment rule remains only for contact reveal.
