...
Desktop controls are WASD/arrows, pointer aim/fire, Space dash, R reload, and E interact. Touch-first layouts expose directional, Use, fire, and dash buttons in safe-area-aware thumb zones. The HUD exposes health, class resource, danger/PvP state, actions, latency, reconnect state, death, and respawn. Home authentication and character creation remain in modal context. The in-game menu explicitly states that the world does not pause.
