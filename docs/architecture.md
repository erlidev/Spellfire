...
The tables cover simulation rates, session windows, common entity defaults, world geometry and danger bands, fixed fixtures, the player body and universal dash, damage bands, elements, abilities, status effects, weapons, spells, blueprint/component layouts, material grades, mobs, biomes, outposts, retired IDs, and the developer-mode spawn/attribute catalog. Rows exist only where a design document has settled the content: the two starter items are live, while component, material, and biome-placement rows arrive with the phases that consume them.
...
Backend tests additionally cover typed entity overrides, tree destruction, undestroyable square-wall behavior, circle/AABB collision, effective-damage ledgers, most-contribution credit, deterministic ties, shield/overkill exclusion, combat-log cursors and life resets; windup cost/delivery, locked geometry, phase transitions and death cancellation; all four validated telegraph figures; every expanded entity/input wire field; and the 64 KiB dense-snapshot guardrail. Frontend tests cover circle/AABB prediction and collision removal, expanded wire decoding, interact encoding, movement/dash prediction, shared-table derivation, and the pending/active/resolved opacity grammar including its resolution flash.
