...
The server interest area covers the complete axis-aligned square whose half-width and half-height equal the configured maximum view distance. Corners of the rectangular camera therefore do not lose entities to a circular range cutoff. Inside that area, terrain marked `occludes_vision` removes actors, projectiles, telegraphs, and fields whose direct sightline it crosses. Those occluded wedges receive a restrained 18%-opacity dark overlay—not an opaque fog—while entities marked `visible_in_shadow`, including trees and walls, remain above it so the source and shape of cover stay legible. Destroying or expiring the blocker reveals what was behind it immediately, while its visual fade finishes.
