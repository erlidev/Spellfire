...
func rebalanced(t *testing.T, damagePerHit float64) *tuning.Tables {
	...
	if err := json.Unmarshal(files["tuning/combat.json"].Data, &document); err != nil {
		t.Fatalf("unmarshal combat.json: %v", err)
	}
	document["damage_bands"].(map[string]any)["standard"].(map[string]any)["damage_per_hit"] = damagePerHit
	raw, err := json.Marshal(document)
	if err != nil {
		t.Fatalf("marshal combat.json: %v", err)
	}
	files["tuning/combat.json"] = &fstest.MapFile{Data: raw}
...
func TestOneTableRowRebalancesBothClassesWithoutCharacterMigration(t *testing.T) {
	...
	if diff := []model.Character{gunslinger, mage}; diff[0] != stored[0] || diff[1] != stored[1] {
